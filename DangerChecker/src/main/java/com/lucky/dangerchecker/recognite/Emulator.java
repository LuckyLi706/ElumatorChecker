package com.lucky.dangerchecker.recognite;

import static android.content.Context.SENSOR_SERVICE;
import static com.lucky.dangerchecker.utils.CommonUtils.isBluePackageName;
...
import com.lucky.dangerchecker.utils.CommandUtils;
...
public class Emulator {
    ...
    private final String[] bsAppName = {FileUtil.getSdCardPath("Android/data/com.bluestacks.home"),
            FileUtil.getSdCardPath("Android") + "/data/com.bluestacks.settings", FileUtil.getSdCardPath("windows/BstSharedFolder")
            , "/data/app-lib/com.bluestacks.settings"};
    private final String[] bsAppPackage = {"com.bluestacks.appmart", "com.bluestacks.settings"};

    //MuMu模拟器
    private static final String STRING_TYPE_MUMU = "MUMU模拟器";
    @SuppressLint("SdCardPath")
    private final String[] mmAppName = {"/data/data/com.mumu.launcher", "/data/data/com.mumu.store",
            "/data/data/com.netease.mumu.cloner", "/data/dalvik-cache/profiles/com.mumu.launcher"
            , "/data/dalvik-cache/profiles/com.mumu.store"};
    private final String[] mmAppPackage = {"com.mumu.store", "com.mumu.launcher", "com.mumu.audio"};

    //逍遥模拟器
    private static final String STRING_TYPE_XIAOYAO = "逍遥模拟器";
    @SuppressLint("SdCardPath")
    private final String[] xyAppName = {"/data/data/com.microvirt.launcher",
    ...
    private final String[] xyAppPackage = {"com.microvirt.launcher", "com.microvirt.download", "com" +
    ...
    @SuppressLint("SdCardPath")
    private final String[] ttAppName = {"/data/data/com.tiantian.ime", "/system/priv-app" +
            "/TianTianLauncher/TianTianLauncher.apk", "/init.ttVM_x86.rc", "/ueventd.ttVM_x86.rc"
            , "/fstab.ttVM_x86", "/system/bin/ttVM-prop"};
    private final String[] ttAppPackage = {"com.tiantian.ime"};
    ...
    @SuppressLint("SdCardPath")
    private final String[] ysAppName = {"/storage/emulated/legacy/BigNoxHD", "/lib/libnoxd.so",
            ...
            ".store" + ".hd"};
    private final String[] ysAppPackage = {"com.bignox.google.installer", "com.bignox.app.store.hd"};

    //Genymontion模拟器
    private static final String STRING_TYPE_GENY = "Genymontion模拟器";
    @SuppressLint("SdCardPath")
    private final String[] geAppName = {"/data/data/com.google.android.launcher.layouts.genymotion",
            "/system/app/GenymotionLayout/GenymotionLayout.apk", "/dev" + "/socket/baseband_genyd"
            , "system/bin/genymotion-vbox-sf"};
    private final String[] geAppPackage = {"com.genymotion.superuser", "com.genymotion.genyd", "com" +
            ".genymotion.systempatcher"};

    //itools模拟器(附属于genymontion)
    private static final String STRING_TYPE_ITOOLS = "itools模拟器";
    @SuppressLint("SdCardPath")
    private final String[] itoolsAppName = {"/data/data/cn.itools.vm.launcher",
            "/data/data/cn.itools" + ".avdmarket", "/data/data/cn.itools.vm.proxy"};
    private final String[] itoolsAppPackage = {"cn.itools.vm.launcher", "Emulator",
            "cn" + ".itools.vm" + ".proxy"};

    //LDS模拟器（鲁大师模拟器）
    private static final String STRING_TYPE_LDS = "LDSBOX";
    private final String[] lbAppName = {"/ldsboxshare", "/init.ludashi.rc", "/init.ludashi.sh"};


    //51模拟器
    private static final String STRING_TYPE_51 = "51模拟器";
    private final String[] w1AppName = {"/mnt/prebundledapps/downloads/51service.apk",
            FileUtil.getSdCardPath("") + ".51service"};

    //海马玩模拟器
    private static final String STRING_TYPE_Drod4X = "海马玩模拟器";
    private final String[] hmwAppName = {"/system/bin/droid4x", "/system/bin/droid4x-prop",
            "/system" + "/lib/libdroid4x.so", "/system/etc/init.droid4x.sh"};
    private final String[] hmwAppPackage = {"com.haimawan.push", "com.haima.helpcenter", "me.haima" +
            ".androidassist"};


    //原生模拟器
    private static final String STRING_TYPE_AS = "AndroidStudio原生模拟器";
    private final String[] asAppName = {"/sys/module/goldfish_audio", "/sys/module/goldfish_sync"};
    ...
    public EmulatorInfo distinguishVM(Context context, int MAX_INDEX) {
        String xiaoYaoTag = isXiaoYao(context);
        String noxTag = isNox(context);
        String blueStacksTag = isBlueStacks(context);
        String fiveOneTag = is51();
        String muMuTag = isMuMu(context);
        String genymotionTag = isGenymotion(context);
        String iToolsTag = isITools(context);
        String tianTianTag = isTianTian(context);
        String droid4xTag = isDroid4x(context);
        String leiDianTag = isLeiDian(context);
        String asTag = isAS(context);
        String otherTag = isOther(context);
        if (StringUtil.getNumber(xiaoYaoTag) >= MAX_INDEX) {
            return new EmulatorInfo(true, STRING_TYPE_XIAOYAO, xiaoYaoTag, "", "");
        } else if (StringUtil.getNumber(noxTag) >= MAX_INDEX) {
            return new EmulatorInfo(true, STRING_TYPE_NOX, noxTag, "", "");
        } else if (StringUtil.getNumber(blueStacksTag) >= MAX_INDEX) {
            if (StringUtil.getNumber(fiveOneTag) > 0) {
                return new EmulatorInfo(true, STRING_TYPE_51, fiveOneTag, "", "");
            } else {
                return new EmulatorInfo(true, STRING_TYPE_BLUESTACKS, blueStacksTag, "", "");
            }
        } else if (StringUtil.getNumber(isMuMu(context)) >= MAX_INDEX) {
            return new EmulatorInfo(true, STRING_TYPE_MUMU, muMuTag, "", "");
        } else if (StringUtil.getNumber(genymotionTag) >= MAX_INDEX) {
            if (StringUtil.getNumber(iToolsTag) > 0) {
                return new EmulatorInfo(true, STRING_TYPE_ITOOLS, iToolsTag, "", "");
            } else {
                return new EmulatorInfo(true, STRING_TYPE_GENY, genymotionTag, "", "");
            }
        } else if (StringUtil.getNumber(tianTianTag) >= MAX_INDEX) {
            return new EmulatorInfo(true, STRING_TYPE_TIANTIAN, tianTianTag, "", "");
        } else if (StringUtil.getNumber(droid4xTag) >= MAX_INDEX) {
            return new EmulatorInfo(true, STRING_TYPE_Drod4X, droid4xTag, "", "");
        } else if (StringUtil.getNumber(leiDianTag) >= MAX_INDEX) {
            return new EmulatorInfo(true, STRING_TYPE_LEIDIAN, leiDianTag, "", "");
        } else if (StringUtil.getNumber(asTag) >= MAX_INDEX) {
            return new EmulatorInfo(true, STRING_TYPE_AS, asTag, "", "");
        } else if (StringUtil.getNumber(otherTag) >= MAX_INDEX) {
            return new EmulatorInfo(true, STRING_TYPE_UNKNOWN, "", otherTag, "");
        } else {
            String tag = xiaoYaoTag + noxTag + blueStacksTag + fiveOneTag + muMuTag + genymotionTag + iToolsTag + tianTianTag + droid4xTag + leiDianTag + asTag + otherTag;
            if (StringUtil.getNumber(tag) >= MAX_INDEX) {
                return new EmulatorInfo(true, STRING_TYPE_UNKNOWN, "", tag, "");
            } else {
                if (tag.contains("1")) {
                    return new EmulatorInfo(false, STRING_TYPE_UNKNOWN, "", tag, "当前阈值高于检测的点,疑似模拟器");
                }
            }
            return new EmulatorInfo(false, "", "", "", "");
        }
    }
     ...
     */
    private String isGenymotion(Context context) {
...
//    }
     ...
     * 基于qumu的模拟器特定属性
     */
    private final String[] known_files = {"/system/lib/libc_malloc_debug_qemu.so", "/sys/qemu_trace",
